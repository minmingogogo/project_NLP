...
# os.path.exists('datasets/all_50_schemas')
with open('datasets/all_50_schemas','rb') as f:
    for l in tqdm(f):
        a = json.loads(l)
...
with open('datasets/train_data.json','rb') as f:
    for l in tqdm(f):
        a = json.loads(l)
        if not a['spo_list']:
            continue
...
with open('datasets/dev_data.json','rb') as f:
    for l in tqdm(f):
        a = json.loads(l)
        dev_data.append(
            {
