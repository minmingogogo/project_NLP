...
1. 带有后缀词训练的模型用于简称场景验证时，行政级别判断不准确：   
   true : 'zon': {'蜀山': array([0, 1]) ; pred :{'cit': {'蜀山': array([0, 1])}    _（测试场景1,1) _   
...
性能方面：    
对learning rate尝试若干数量级的warmup steps 先增长后衰减策略     
我在另一个数据集/Tsinghua_10cate 实体提取任务上进行了warmup steps 的对比，效果图在：/task2_ner/imgs/ ts_bert_warmup_steps.png    
  
## 总结
1. 密集型实体语料：      
本数据集来源于物流地址，较一般的实体任务，这是一个实体密集型数据集合。语料中实体密集，则模型会学到实体实体间顺序关系，预测时也会基于字符位置作出实体预测，如结果分析第3，4点所述。    
2. 模型的特征词学习：    
13种干扰场景中，测试了行政级别简称模型的效果，训练集是规范的 五级+关键字 带后缀词全称组合，由结果分析第1点中的例子可以看出，模型可以提取实体"蜀山" 但信息缺失较多其判断类别出错。  

3. 模型的结构学习：    
以上13种干扰场景设计在与实际业务场景一致，从前文模型健壮性评估中可以看出，   
十三种场景验证下，模型在行政区划前后颠倒后表现最差，实体类型的排列顺序即文本的结构对模型影响最大。   
这种结构学习能力在对实体关系提取，语义依存任务也适用。 
从另一方面讲，crf layer 非常好的学习了语料的实体关系/结构，因而在乱序场景表现不佳。
